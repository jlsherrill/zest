...
> map[string]interface{} DocsApiJsonGet(ctx).XTaskDiagnostics(xTaskDiagnostics).Lang(lang).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.DocsApiJsonAPI.DocsApiJsonGet(context.Background()).XTaskDiagnostics(xTaskDiagnostics).Lang(lang).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {
